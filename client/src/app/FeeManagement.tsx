        ...
        <section className="space-y-6">
          ...
          <div className="grid gap-3">
             {categories.map((cat, idx) => (
               <div key={cat._id || `cat-${idx}`} className="p-5 rounded-3xl bg-card border border-white/5 flex items-center justify-between group hover:border-primary/20 transition-all">
             ...
             <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-white/5">
                   {structures.map((struct, idx) => (
                     <tr key={struct._id || `struct-${idx}`} className="hover:bg-white/5 transition-colors">
                        <td className="px-6 py-4 text-sm font-bold text-slate-300">{struct.classId?.name}</td>
             ...
             >
                ...
                <div className="space-y-4">
                  <div className="space-y-1.5">
                    ...
                    <select 
                      title="Academic Year"
                      className="w-full bg-white/5 border border-white/5 rounded-xl px-4 py-3 text-sm font-bold text-white outline-none focus:border-primary/50 transition-all"
                      value={newStructure.academicYearId}
                      onChange={e => setNewStructure({...newStructure, academicYearId: e.target.value})}
                      required
                    >
                      <option value="">Select Year</option>
                      {academicYears.map((y, idx) => <option key={y._id || `y-${idx}`} value={y._id}>{y.name}</option>)}
                    </select>
                  </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    >
                      ...
                      {categories.map((c, idx) => <option key={c._id || `c-opt-${idx}`} value={c._id}>{c.name}</option>)}
                    </select>
                  </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    >
                      ...
                      {classes.map((c, idx) => <option key={c._id || `cls-${idx}`} value={c._id}>{c.name}</option>)}
                    </select>
                  </div>
